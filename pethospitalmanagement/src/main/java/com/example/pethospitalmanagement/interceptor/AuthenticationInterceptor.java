package com.example.pethospitalmanagement.interceptor;
...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(clazz.isAnnotationPresent(UserLoginToken.class)){
            ...
            if (userLoginToken.requirea()) {
                // 执行认证
                if (token == null) {
                    throw new MyException("E","无token，请重新登录");
                }
                ...
                try {
                    ...
                    throw new MyException("E","token异常，请重新登录");
                }
                User user = userMapper.selectByPhone(phone);
                if (FormTools.isNull(user)) {
                    throw new MyException("E","用户不存在，请重新登录");
                }
                // 验证 token
